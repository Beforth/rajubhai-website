...
- **What's still full-bleed, deliberately:** just the Hero photo now (fills half the 100vh split, no frame, no gutter). About's photo used to bleed all the way to the viewport edge too, but client feedback ("the img is too right... proper whitespace on left and right") called that out specifically — a photo with zero right margin looked unbalanced next to a text column that (correctly) respects the page's growing side gutter on wide screens. Fixed by giving `.about-photo-bleed` a `padding-right` using the exact same formula as everyone else's right edge (`max(32px, calc((100vw - 1200px)/2 + 32px))`), and `padding:0 32px` at the ≤980px stacked breakpoint — the peach section background shows through as a deliberate margin rather than the photo touching the edge. Hero's photo is unaffected and still genuinely full-bleed; that one wasn't part of the complaint and a big banner photo bleeding edge-to-edge reads as normal, not unbalanced, the way a smaller content photo does.
...
| [About.astro](src/components/About.astro) | Bold `--peach` block, flat paper pull-quote card, icon-roundel points. The photo is no longer full-bleed to the viewport edge — it now carries a right-side gutter matching every other section's right edge (see Layout conventions). |
