...
| [Hero.astro](src/components/Hero.astro) | H1 + count-up stat strip (1987 / 30+ / 4.8) + photo card that fills its grid column (no artificial width cap). Its own signature details: a faint "1987" watermark behind the text column (reuses About's `.watermark` pattern — the two sections now bookend the page with the same motif), a red highlighter-style bar behind "Dabeliwale" (`em::after`, `z-index:-1`, echoes the pull-quote's red bar), and a thin `--orange` inset ring inside the photo frame (`.frame-card::before`, `inset:6px`) |
...
| [Menu.astro](src/components/Menu.astro) | Dark `--ink` section, receipt-style card with ticket-notch cutouts. The three categories (Dabeli / Fafda & Chaat / Sandwiches, Burgers & More) sit in a 3-column grid inside the receipt — one column each, hairline dividers between them, each with a small line-icon next to its title — collapsing to a single stacked column (top-border dividers instead of side ones) at ≤980px. Items carry descriptions, a `Signature` badge on 3 of them, and a price column (currently `₹—` placeholders — real prices pending from the client). Originally one long stacked list of all 10 items; that made the receipt very tall and visually monotonous, so it was split into the per-category columns below. |
