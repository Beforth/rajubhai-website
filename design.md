...
- **Nothing is full-bleed to the true viewport edge anymore.** Both Hero's and About's photos used to bleed flush right with zero margin. Client feedback first called this out on About ("the img is too right... proper whitespace on left and right"); when Hero turned out to have the identical issue, feedback followed ("fix the first and second section") to fix it there too. Both `.hero-photo` and `.about-photo-bleed` now carry the same `padding-right: max(32px, calc((100vw - 1200px)/2 + 32px))` — the exact formula every other section already uses for its own right edge — plus `padding:0 32px` at the ≤980px stacked breakpoint. The section's own background (paper for Hero, peach for About) shows through as a deliberate margin instead of the photo touching the edge. **If a future section wants a genuinely full-bleed photo, that's now a deviation from an established page-wide rule, not the default — confirm it's actually wanted before building it that way.**
...
- **About is now mirrored — photo left, text right** (client: "move ht pic to left and info to right"), the opposite of Hero's text-left/photo-right, for zig-zag rhythm between the two full-bleed sections. Done with CSS `order` on the two grid children (`.about-text{order:2}`, `.about-photo-bleed{order:1}`), not by reordering the markup — so DOM/reading order for screen readers stays text-then-photo regardless of the visual swap. The bleed gutter moved with it: `.about-photo-bleed` now carries `padding-left` (was `padding-right`) with the same formula, since the photo's outer edge is now the left side of the section.
  - **Same request also flagged real cropping** ("dont cut the img"): the photo column was `align-items:stretch`-ed to match the text column's full height, then `object-fit:cover` cropped hard into the sides of this landscape photo (1600×1067) to fill that tall, narrow box. Fixed at the source instead of hiding it with `contain` (which would've letterboxed badly against such a mismatched box ratio): `.about-photo-bleed{align-self:center}` (no longer stretched to row height) + the image itself sized as `width:100%; height:auto; aspect-ratio:1600/1067` off its own post-padding content width. The box now takes its height from the image's real ratio, so `cover` has nothing left to crop. Verified `imgRect.width/height` matches `1600/1067` exactly at 1920/1366/375px.
...
| [About.astro](src/components/About.astro) | Bold `--peach` block, flat paper pull-quote card, icon-roundel points. Photo left, text right (mirrors Hero) via CSS `order`; the photo's own left-side gutter matches every other section's edge formula, and the photo box is sized to its real aspect ratio instead of being stretched/cropped (see Layout conventions). |
